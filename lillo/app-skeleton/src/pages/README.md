...
- `AlertDetailPage` now checks a local diagnosis preset map first; only the vineyard `Peronospora risk` alert switches to the diagnosis-flow card with explicit trigger evidence, pattern explanation, data-used rows, and action checklist, while all other alerts keep the existing generic detail rendering path.
