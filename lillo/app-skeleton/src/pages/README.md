...
- Alert opens from both `AlertsPage` and dashboard warning tiles now preserve the current shell preview query (`mode=phone|desktop|roadmap`) through `/alerts/:alertId -> /alert`, so desktop preview does not fall back to phone when entering alert detail.
