...
- `AppShell.jsx` (`DRAFT`) - owns shell composition, preview-mode state, URL sync for `mode` search params, mode toggle UI, the shell-level `Consorzio page` entry button, phone-intro replay/skip controls, stage-level click/space trigger for the terminal pre-intro, measurement of the centered phone/stage bounds for overlay positioning, terminal phase transitions through `visible -> sources -> feeding -> running -> collapsing -> bubble -> connectingToPhone -> handoffToPhone`, the intro-to-phone trigger callback, the current three-item primary nav (`Dashboard`, `Alerts`, `Alert`) with icons, and stage wrapper selection (`PhoneFrame`, desktop preview frame, or Roadmap presentation mode).
- `AppShell.css` (`DRAFT`) - styles for the outer shell (toggle + preview stage), including the decoupled phone-anchor layer, measured intro overlay layer, the shared pill styling used by the consorzio entry link and intro controls, absolute phone centering that never shares layout flow with the intro, the desktop-preview-only side rail layout for desktop nav, and the unclipped stage layering needed for the outgoing bubble-to-phone connector.
- `RoadmapPresentation.jsx` (`DRAFT`) - dedicated presentation-mode canvas rendered only in `Roadmap` mode; owns step state, click/keyboard progression, animated Company -> Consorzio -> many farmers sequence, top-farmer identikit burst, the cost/time overlay stage, and the final consorzi-reach map expansion stage.
- `RoadmapPresentation.css` (`DRAFT`) - full-bleed roadmap canvas styling plus node/line reveal animations, top-farmer focus treatment, identikit burst tokens, floating arrow controls, compact cost/time panel visuals, and the final consorzi reach-map transition card.
...
- The shell top toggle area now includes a `Consorzio page` link, but that control performs route navigation only; the standalone consorzio routes still bypass the shell entirely instead of reusing preview-mode state.
...
- `PreviewApp` nav links always append the current `mode` query (`phone`, `desktop`, or `roadmap`) so route switches from the shell nav keep the active preview context instead of falling back to phone mode.
- Phone/Desktop preview switching now uses a shared two-panel carousel track in `AppShell` (while Roadmap still renders separately), so both preview surfaces remain mounted and the inactive panel is marked `inert` to prevent off-screen interaction while preserving the slide animation state.
...
- `RoadmapPresentation` advances by stage click/tap, keyboard (`ArrowLeft`, `ArrowRight`, `Enter`, `Space`), or floating arrow controls; step progression is persisted in `sessionStorage` (`app-shell-roadmap-step`) so revisiting Roadmap mode restores the latest stage.
...
- The step immediately after `Costi` now shows a two-phase market-reach card: first it stays on `Trentino` using `map-big.png` (the file names are inverted to the visual zoom level) with `458 consorzi`, then the next manual advance switches to `Nord Italia` using `map-little.png` with `25.000 consorzi`; the adjacent right metric uses the same card style and derives reached farms as `consorzi * 50`, and both numbers climb with a slot/jackpot-style roll on each phase.
...
- The `Costi` overlay remains in-sequence, and the new final roadmap step keeps the same canvas while shifting focus to scalable consorzio distribution (local -> Nord Italia).
