...
- `AlertDiagnosisFlow.jsx` (`DRAFT`) - diagnosis-flow detail renderer used for disease-risk style alerts; it owns the guided sequence `dati -> problema identificato -> soluzione` with severity/timestamp hero, plant visual, evidence chips, integrated reasoning, and action checklist from a profile object so new presets (for example Freeze Risk) can be added without changing layout structure.
...
- `AlertDiagnosisFlow` stays intentionally risk-oriented (mai diagnosi certa), localizes relative timestamp labels for the detail hero, and applies its strongest layout compaction inside the existing mobile breakpoint (`max-width: 700px`) so desktop structure remains close to the current alert-detail flow.
