...
- `AlertDiagnosisFlow.jsx` (`DRAFT`) - diagnosis-flow detail renderer used for disease-risk style alerts; it owns the guided sequence `dati -> problema identificato -> soluzione` with severity/timestamp hero, plant visual, evidence chips, integrated reasoning, and a single action section that can render a desktop timeline (numbered bubbles + connector line + staggered reveal) plus mobile compact fallback from a profile object so new presets (for example Freeze Risk) can be added without changing layout structure.
...
- `AlertDiagnosisFlow` stays intentionally risk-oriented (mai diagnosi certa), localizes relative timestamp labels for the detail hero, and now activates the desktop action timeline with an `IntersectionObserver` when the action section enters view: numbered bubbles and text reveal in stagger, the connector line fills progressively, the closing risk note stays in the same action container (no separate impact card), and mobile (`max-width: 700px`) keeps a simpler stacked action fallback.
