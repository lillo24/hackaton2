...
- `App.jsx` - top-level route wiring, fixed profile context selection, standalone admin route registration, and app-level selected-alert state owner.
...
- `layout/` - preview stage and shell wrappers for phone/desktop routes, plus standalone roadmap presentation mode and the shell-level admin entry button.
- `pages/` - route-level screens for dashboard, alerts, dedicated alert detail, and the standalone admin surfaces.
- `data/` - centralized shared entities plus selectors for farm-aware alert derivation and the local mock admin dataset.
...
- `/admin` and `/admin/customers/:customerId` are intentionally routed outside `AppShell`, so the imported admin UI stays a standalone page flow instead of becoming another farmer preview mode.
